...
st.set_page_config(
    ...
    page_icon="📊",
    layout="wide",
    initial_sidebar_state="expanded"  # 改為默認展開側邊欄
)
...
def fetch_etf_data_from_yahoo(etf_list):
    """從Yahoo Finance獲取ETF數據"""
    result_data = []
    
    for etf_code in etf_list:
        try:
            # 格式化ETF代碼以適應Yahoo Finance
            ...
            if not hist.empty:
                ...
                # 將時區感知的datetime轉換為naive datetime以進行比較
                if not dividends.empty:
                    # 將索引轉換為Python datetime對象
...
def show_sidebar():
    ...
    st.sidebar.markdown("本工具使用自動爬蟲從以下來源獲取數據：")
