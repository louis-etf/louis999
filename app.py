...
# 以下為原始功能代碼，功能部分保持不變
#########################################
# （存股計算、ETF配息分析器、爬蟲更新等功能均保持原狀，只調整CSS配色）
...
def plot_investment_growth(years, values):
    ...
    fig.update_layout(
        title=dict(text='累積金額成長曲線', font=dict(size=18, color='#2c3e50'), y=0.95),
        plot_bgcolor='white',
        paper_bgcolor='white',
...
def dividend_calculator():
    ...
    with col1:
        lump_sum = st.number_input("一次投入 (NT$)", min_value=0, value=1000000, step=100000)
        etf_code = st.text_input("存股標的", value="00788B")
    with col2:
        annual_div_yield = st.number_input("預估股息殖利率(%)", min_value=0.0, max_value=100.0, value=5.0, step=0.1)
        bank_rate = st.number_input("銀行存款利率(%)", min_value=0.0, max_value=15.0, value=1.73, step=0.01)
    if st.button("開始試算", key="dividend_start"):
        annual_div = lump_sum * (annual_div_yield / 100)
        monthly_div = annual_div / 12
        st.write("---")
        st.write(f"每年可領股息：約 **NT${annual_div:,.0f}**")
        st.write(f"每月可領股息：約 **NT${monthly_div:,.0f}**")
        bank_annual = lump_sum * (bank_rate / 100)
        st.write(f"銀行每年利息：約 NT${bank_annual:,.0f}（利率 {bank_rate:.2f}%）")
        st.write("此數值可作為投資ETF vs. 存銀行之參考。")
